...
#Vybereme libovolný okres z https://www.volby.cz/pls/ps2017nss/ps3?xjazyk=CZ.
...
#Funkce pro zjištění názvu obcí vybraného okresu
...
#Funkce pro zjištění čísla kódů obcí vybraného okresu
...
#Funkce pro zjištění url výsledků hlasování obcí vybraného okresu
def url_okrsku_fce(url_okres):
    ...
    for i in range(0, len(obec_kod_pom)):
        for a in obec_kod_pom[i].find_all('a'):
            url_obec.append('https://www.volby.cz/pls/ps2017nss/' + a['href'])
    return url_obec

#Funkce pro zjištění početu voličů v obcích vybraného okresu
...
#Funkce pro zjištění početu vydaných obálek v obcích vybraného okresu
...
#Funkce pro zjištění početu platných hlasů v obcích vybraného okresu
...
#Funkce pro zjištění početu platných hlasů jednotlivých stran v obcích vybraného okresu
...
#Funkce pro zjištění čísla jednotlivých stran v obcích vybraného okresu
...
def volby_celkem_fce(url, vystup_csv):
    ...
    if ".csv" not in vystup_csv:
        ...
        if "https://www.volby.cz/pls/ps2017nss/ps3?xjazyk=CZ" not in url:
